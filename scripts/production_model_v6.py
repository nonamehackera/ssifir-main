...
# home_score_prob etc are 0.5 for 88% of rows (proxy) - useless
...
# Sample weights: kullanilmadi (home bias football'da normal)
sample_weights = np.ones(len(ytr))

# LGB x3 - HOME BIAS FIX: sample_weight ile dengesizligi duzelt
lgb_models, lgb_ws = [], []
for s in [42, 123, 789]:
    t1 = time.time()
...
for s in [42, 123, 789]:
    ...
    sample_weights = np.where(ytr == 0, 1.0, np.where(ytr == 1, 2.0, 1.3))
...
print(f"  (Walk-forward sonuclari icin: python scripts/walk_forward_run.py)")
