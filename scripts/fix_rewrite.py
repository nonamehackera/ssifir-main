...
# _predict_match govdesi: 1174..1316 (1-indexed) = 0-indexed 1173..1315
# Bunlari tamamen temiz bir govdeyle degistir.
start = 1173
end = 1316

new_body = '''    home_form = hd.get("form", "?????")
    away_form = ad.get("form", "?????")

    row = {}
    skip_cols = {"match_id", "date", "referee", "result", "home_goals", "away_goals",
                 "total_goals", "btts", "over25", "home_team", "away_team",
                 "home_corners", "away_corners"}
    for col in feat.columns:
        if col in skip_cols:
            continue
        if col.startswith("away_") and col in a_row.index:
            row[col] = a_row[col]
        elif col in h_row.index:
            row[col] = h_row[col]
        else:
            row[col] = 0
    ...
    row["home_elo"] = hd.get("elo", 1500)
    row["away_elo"] = ad.get("elo", 1500)
    row["elo_diff"] = hd.get("elo", 1500) - ad.get("elo", 1500)
    row["home_attack_elo"] = hd.get("attack_elo", 1500)
    row["home_defence_elo"] = hd.get("defence_elo", 1500)
    row["away_attack_elo"] = ad.get("attack_elo", 1500)
    row["away_defence_elo"] = ad.get("defence_elo", 1500)
    row["attack_elo_diff"] = hd.get("attack_elo", 1500) - ad.get("attack_elo", 1500)
    row["defence_elo_diff"] = hd.get("defence_elo", 1500) - ad.get("defence_elo", 1500)

    if "league" not in row or not row["league"]:
        row["league"] = hd.get("league_code", "E0")
    if "season" not in row or not row.get("season"):
        row["season"] = "2025"
...
print("rewritten _predict_match body")
